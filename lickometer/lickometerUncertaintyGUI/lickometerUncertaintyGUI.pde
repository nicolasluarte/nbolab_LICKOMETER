...
int[][] dtubeReward = new int [4][3]; // show if the event was rewarded or not
...
void mainMenu() {
  ...
  text("Presione los numeros para acceder a menus", 100, yref + 50);
  textSize(20);
  textAlign(LEFT, BOTTOM);
...
void portMenu() { 
  ...
  fill(0); 
  textSize(18); 
  text(s, 600, 50, 300, 100);  // Text wraps within text box
...
void configMenu() { 
  ...
  if (index != 0) {
    ...
    for (int i = 0; i < index; i++) {
      drawConfigPanel(i, myPorts[myArdIndex[i]], xRefConfigMenu[i], yRefConfigMenu[i]); // First argument indicates number of arduino
    }
    ...
    textAlign(CENTER);
    text("ENVIAR CONFIGURACION", 5, 150, 200, 75);
    ...
    textAlign(CENTER);
    text("PROBAR BOMBAS", 5, 240, 225, 75);
  } else {
...
void testMenu() {  
  ...
  if (index != 0) {
    ...
    for (int ardIndex = 0; ardIndex < index; ardIndex++) {
      ...
      drawInfoPanel(ardIndex, myPorts[myArdIndex[ardIndex]], xRefTestMenu[ardIndex], yRefTestMenu[ardIndex]); // First argument indicates number of arduino
    }
  } else {
    text("ASIGNE PUERTOS PARA COMENZAR", width / 2, height / 2);
  }
...
void drawInfoPanel(int ardIndex, String Port, int xref, int yref) {
  ...
  textAlign(LEFT, BOTTOM);
  textFont(f, 18); // Specify font to be used
  rect(xref, yref, 560, 275); // Create rectangle to contain information
  fill(0); 
  text("ARDUINO " + ardIndex, xref + 10, yref + 20); // Include port
  fill(0); 
  text(Port, xref + 10, yref + 40); // Include port
  ...
  fill(10, 100, 27);
  if (firstContact[ardIndex] == false) { 
    fill(127, 0, 0); 
    status = "NO INICIADO...";
  }
  if (firstContact[ardIndex] == false && requestContact[ardIndex] == true) { 
    fill(127, 0, 0); 
    status = "CONECTANDO...";
  }
  if (pause[ardIndex] == 1 && firstContact[ardIndex] == true) { 
    fill(127, 0, 0); 
    status = "PAUSA...";
  }
  ...
  text("#recompensa", xref + 480, yref + yref2);
  ...
  fill(0); 
  for (int i = 0; i < 3; i++) { 
    text(dtubeReward[ardIndex][i], xref + 525, ypos[i]);
  }
  ...
  fill(80, 80, 100); 
  rect(xref + 175, ypos[2] + 20, 150, 20); 
  fill(200, 0, 0); 
  text(" START / PAUSA", xref + 190, ypos[2]+38);
...
void serialEvent(Serial p) {  
  ...
  if (message != null) {   
    ...
    if (data[0].charAt(0) == HEADER && data.length == 14) {      // check for header character in the first field       
      ...
      int ardIndex = 0; 
      String flag = "0";                  
      if (p == mySerialPorts[0] && pause[0] == 0 && firstContact[0] == true) { 
        ardIndex = 0; 
        flag = "1";
      }
      if (p == mySerialPorts[1] && pause[1] == 0 && firstContact[1] == true) { 
        ardIndex = 1; 
        flag = "1";
      }
      if (p == mySerialPorts[2] && pause[2] == 0 && firstContact[2] == true) { 
        ardIndex = 2; 
        flag = "1";
      }
      if (p == mySerialPorts[3] && pause[3] == 0 && firstContact[3] == true) { 
        ardIndex = 3; 
        flag = "1";
      }
...
void controlEvent(ControlEvent theEvent) {
  if (theEvent.isController()) { 
    if (theEvent.getController().getName() == "INICIO") { 
      startTime = int(theEvent.getController().getValue());
    }
    if (theEvent.getController().getName() == "TERMINO") { 
      endTime = int(theEvent.getController().getValue());
    }
  }
}
